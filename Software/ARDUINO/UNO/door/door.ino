...
void loop() {   
  ...
  {   
    ...
    sprintf(ReplyDiscover,"%s%s%s%s%s%s%s%s%s%s",idd,id,sp,net,sp,type,sp,state,sp,lock);
    ...
    Serial.println("Msg: Discovery "); 
    ...
    else if (strcmp(packetBuffer,"SET LOCK") == 0){
      char lock[]= "1";
      sprintf(ReplyBuffer,"%s%s",ack,lock);
      Udp.write(ReplyBuffer);
    }
    else if (strcmp(packetBuffer,"SET UNLOCK") == 0){
      char lock[]= "0";
      sprintf(ReplyBuffer,"%s%s",ack,lock);
      Udp.write(ReplyBuffer);
    }
      ...
      char ReplyRefresh[]="REF";
     // + STATE;
      Udp.write(ReplyRefresh);
    }
